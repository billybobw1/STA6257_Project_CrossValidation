...
### Single Hold-Out Method:
This is one of the more simple methods of
...
### K-Fold Cross-Validation:
The dataset is split into k equally sized
...
### Stratified Cross-Validation:
Ensures each fold reflects the class proportions in the dataset, particularly
...
Zhang and Yang (2015) focus on cross-validation as a model selection method.
 ...
 - Cross-validation techniques can be used for model selection in high-dimensional settings
 - Model parameters can be selected through the use of cross-validation (ex., Akaike Information Criterion (AIC) vs. Bayesian Information Criterion (BIC))
 - Highlight the importance of selecting an ideal data-splitting ratio for the dataset
...
- The correlation between replications of k-folds should be considered before increasing the number of replications.
- Experimentation across 20 datasets
- It is recommended to use a larger number of folds with fewer replications
...
- The goals of rdCV are as follows:
  ...
  + Provide more accurate prediction error estimates for future models
...
- The DUPLEX algorithm is employed to split the data into training and testing sets and uniformly distribute the farthest data
- K-Fold cross-validation is then performed on the split data
- RSCV was only found to be significantly different from traditional cross-validation techniques in one experiment.
...
- Correction is unnecessary when distribution of data is not different when between training and testing datasets.
- The performance of CVc was compared to that of basic cross-validation on two datasets: one with a cluster of correlation structure, and one with a hierarchical spatial correlation structure.
- CVc more accurately estimated generalization error, remaining unbiased as long as the covariance structure was specified.

## Literature Review - [@lei2020cross]

Lei (2020) introduce Cross-Validation with Confidence (CVC) to try to reduce overfitting.

- CVC uses the predicted risk of candidate models and their residuals to fit models.
  + This method can be used for model fitting as well as parameter tuning.
- CVC can be computationally complex, but Lei claims the running time is comparable to other cross-validation methods.
- When compared to standard cross-validation, CVC was able to produce an adequate predictive model with fewer predictors and better interpretibility.
- CVC had not been considered for unsupervised methods or binary outcome variables at the time of publication.
...
Another performance metric that will be used to determine model performance is the area under the ROC curve (AUC). AUC conveys how well a model can differentiate between different classes, and it is preferable to other metrics, such as accuracy, because it accounts for both specificity and sensitivity [@cantor2000determining]. AUC scores can range from 0 to 1, with 0 indicating the model has no ability to discern between classes, and 1 indicating the model perfectly predicts classification.

## Performance Metrics - AUC (cont.)

Figure 6 demonstrates how AUC uses the positive and negative outcomes to calculate a value that indicates the probability of a positive result ranking higher than negative results, where $S_p$ is the total number of positive outcomes ranked, and $n_p$ and $n_n$ are the number of positive and negative outcomes, respectively [@hossin2015review].
...
:::{.column width="40%"}
- The dataset contains 13 clinical, body, and lifestyle features from 299 heart failure patients in Faisalabad, Pakistan in 2015
- The goal was to demonstrate potential biomarkers in predicting heart failure outcomes
- The study focused solely on serum creatinine and ejection fraction as predictors of patient survival
:::
...
- The time variable is the number of days in the
...
[@chicco2020machine]. 
- The serum creatinine is created by this creatinine
when muscle breaks down [@chicco2020machine], so this variable was
included as another potential indicator of injury.
- Both creatinine features indicate some right-tailed skewness
:::
...
:::{.column width="40%"}
- Variables were included that observe the levels of sodium (serum sodium) and number of
platelets in the patient's blood. 
- The ejection fraction variable is the
percentage of blood leaving the left ventricle during each contraction
of the heart [@chicco2020machine].
- These features look fairly normally distributed, with some potential skewing in the platelet and serum sodium features.
:::
...
- These variables include whether the patient is
a smoker, has diabetes, anaemia, or high blood pressure. 
- The sex variable indicates whether the patient is a female ('0') or male ('1').
- Our outcome variable, death event, indicates whether the patient died
...
analysis to evaluate if any variables have outliers that need to be
addressed. 
- However, these data points will remain untouched for our analysis as they do
not appear to be recording errors. The outliers in the boxplots appear to be genuine
observations and should be considered in our model.
:::
...
::: {.column width="30%}
- The most significant correlation was between the time
...
period tended to survive through the end of the follow-up period. 
- There was a slight positive correlation between sex of the patient and smoker
...
Since the outcome variable (death event) is binary, logistic regression will be used with five different cross-validation techniques: 10-fold, 5-fold, single hold-out, leave-one-out, and Monte Carlo cross-validation. Two different models will be fit for each technique: one with every predictor variable included (Model 1), and the other using only serum creatinine and ejection fraction (Model 2), as was advised by [@chicco2020machine]. The goal is to compare how the difference in performance between the two models changes when various cross-validation techniques are applied. The performance of the models will be determined using their MSE and AUC values.

## 10-fold Cross Validation - Model 1

With any variation of k-fold cross validation, the dataset is split into
k sized folds that will all be equal. The function used to create our cross-validation was cv.glm, which only requires your data, the glm model, and the number of folds for the cross validation. Here, the dataset is split in 10 folds. As previously discussed, [@wong2019reliable] mentions 10-fold
cross validation analysis to be most effective in most cases. Our glm model was changed from a linear regression to logistic, since our outcome variable is binary. In model
1, all predictor variables are included. The default metric for cv.glm is MSE, so a cost function was created to calculate the AUC values for the models. This cost function will be used for both k-folds and LOOCV. 
```{r, echo=TRUE}
library(boot)
...
## 10-fold Cross Validation - Model 2

In model 2, only ejection fraction and serum creatinine are included, as the researchers that
provided the data set came to the conclusion that death events could be
predicted using those 2 variables alone.
```{r, echo=TRUE}
# Model 2
set.seed(85329)
...
Interestingly enough, the first model that included all variables produced a lower MSE and a higher AUC, indicating that from this CV test, model one would be better to
use than model 2. Model 2 has an AUC that is still acceptable, but the AUC of the first model is significantly better.
```{r}
...
pieces using cv.glm. However, in this case, we are only doing 5 folds compared to our
10-fold analysis. Again, in model 1, all predictor variables are
included.
```{r, echo=TRUE}
library(boot)
...
In model 2, only ejection fraction and serum creatinine are
included. 
```{r, echo=TRUE}
set.seed(185329)
m2 <- glm(DEATH_EVENT ~ ejection_fraction + serum_creatinine,
...
Model 1 once again performed the best based on its MSE and AUC values. Both AUC values are acceptable, and their MSE's are fairly low. However, model 1's ACU is significantly better.
```{r}
k5tibble = tibble(M1_K5_MSE = k5_m1_mse[2], M2_K5_MSE = k5_m2_mse[2], M1_K5_AUC = k5_m1_auc[2], M2_K5_AUC = k5_m2_auc[2])
k5tibble
```

## Single Hold-Out Method - Test/Train Split & Model Creation

Using the Single Hold-Out Method, the data set is split into a training
set and a test/validation set. Since the cv.glm function only applies to k-fold cross-validation, the splitting of the data will be done manually. Here, we are using a 50-50 split. Our functions for creating each model was identical to our previous techniques, using glm() to perform logistic regression.
```{r, echo=TRUE}
...
## Single Hold-Out Method - MSE & AUC Calculation
...
## Single Hold-Out Method - Performance

After conducting the analysis, it can be seen that model 2 provides us with a
significantly lower mean squared error. Both models have very similar AUC values, with the first model only producing a marginally higher value. Model 2 includes only ejection fraction and
serum creatinine as predictor variables, meaning this reduced model is a
better fit according to this cross-validation technique. This result aligns with the opinion of the original article this data was sourced from.
```{r}
...
splitting technique to the previous k-fold examples, but in this case, the data is split k-ways, where k
is equal to the number of data points. In this case, k would be 299. Since LOOCV is a form of k-folds, cv.glm can be used for data splitting. The "K=" argument isn't necessary when performing LOOCV, as the function assumes that the number of folds will be equal to the number of observations. Once again, the cost function is used to find our AUC values. The same function (glm()) was used to fit our logistic regression models as the other k-fold techniques. Once again, the processes for both models was identical apart from the number of predictors included in the models.
```{r, echo=TRUE}
...
According to LOOCV, the full model (M1) is a better fit due to a
lower MSE compared to the reduced model (M2). The AUC models follow a similar pattern, with model 1 having a higher AUC than model 2.
```{r}
...
## Monte Carlo Cross-Validation - Resampling Function & Model Creation
...
## Monte Carlo Cross-Validation - Resampling & Metric Calculation

The fit_resamples() function applies our resampling function created earlier to our models to perform our cross-validation. For the MSE of the models, we'll be using the Brier score as our resampling fitting function generates the Brier score automatically, and it performs similarly to MSE in classification problems [@ferro2012bias].
```{r, echo=TRUE}
...
The only technique that stands out as having significantly different results was single hold-out cross-validation. It's the only technique that clearly indicated the reduced model as the superior model. The significant difference leads us to believe the simplicity of the technique could have caused some overfitting and higher bias. Other training/testing split ratios could be considered to try to combat this.
The rest of the techniques performed so similarly with all measures considered, that choosing the technique that best fit and predicted our data is difficult.
...
ejection fraction as predictor variables. The logistic regression analysis
...
dataset) tends to have lower mean squared error (MSE) and higher Area under the Curve (AUC) values after testing. This essentially means the full model is more robust with predictive ability
...
technicality does have slightly more predictive ability. It also has a better ability to distinguish between classes, whether or not the patient died within their follow-up period.
...
typically preferred in statistical analysis. Since the AUC values did show differences between models, the model could perhaps benefit from some additional predictors. However, the reduced models still demonstrated acceptable class discrimination.

Single hold-out cross-validation stood out the most as having the worst performance, with MSE values that were significantly higher than that of the rest of the techniques. This result indicates that this technique will not always be the best route to take when trying to validate a model. The rest of the models had only marginal differences in performance. However, when considering the process of administering the techniques, the k-folds, including leave-one-out, provided the most straight-forward approach. Having a function that performed the cross-validation from the logistic regression model removed some of the opportunity for human error. Ultimately, most of these techniques seem sufficient to
