...
as a test set. In a sense, Leave-One-Out Cross-Validation (LOOCV) is equivalent to setting k=1 in k-Fold Cross-Validation.
...
MCCV randomly removes significant chunks of the dataset without replacement to use as its training and
...
- Observations could be used in the validation set multiple times or not at all
- Reduced computational complexity than other techniques (ex., LOOCV) [@james1980monte]
...
In model 1, all predictor variables are included. The default metric for cv.glm
...
our logistic regression models as the other k-fold techniques.

## LOOCV (cont.)

Once again, the processes for both models was identical apart from the number
