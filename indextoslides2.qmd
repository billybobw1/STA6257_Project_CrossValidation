...
Ensures each fold reflects the class proportions in the dataset,
...
-   AUC conveys how well a model can differentiate between different
    ...
    because it accounts for both specificity and sensitivity
    [@cantor2000determining].
-   AUC scores can range from 0 to 1

Figure 6 demonstrates how AUC uses the positive and negative outcomes to
...
-   The serum creatinine is created by this creatinine when muscle
    breaks down [@chicco2020machine], so this variable was included as
    another potential indicator of injury.
...
-   All of our categorical variables are Boolean and are encoded as '0'
    and '1' to indicate negative or positive occurrence of a variable's
...
::: {.column width="\"30%"}
-   The most significant correlation was between the time variable and
    our target variable, death event. There's a negative relationship,
    meaning the patients who had more days in the follow-up period
    tended to survive through the end of the follow-up period.
-   There was a slight positive correlation between sex of the patient
    and smoker status, meaning more male patients tended to be smokers
...
regression to logistic, since our outcome variable is binary. In model
1, all predictor variables are included. The default metric for cv.glm
is MSE, so a cost function was created to calculate the AUC values for
the models. This cost function will be used for both k-folds and LOOCV.
...
After conducting the analysis, it can be seen that model 2 provides us
with a significantly lower mean squared error. Both models have very
similar AUC values, with the first model only producing a marginally
higher value. Model 2 includes only ejection fraction and serum
creatinine as predictor variables, meaning this reduced model is a
better fit according to this cross-validation technique. This result
aligns with the opinion of the original article this data was sourced
from.
...
splitting technique to the previous k-fold examples, but in this case,
the data is split k-ways, where k is equal to the number of data points.
In this case, k would be 299. Since LOOCV is a form of k-folds, cv.glm
can be used for data splitting. The "K=" argument isn't necessary when
performing LOOCV, as the function assumes that the number of folds will
be equal to the number of observations. Once again, the cost function is
used to find our AUC values. The same function (glm()) was used to fit
our logistic regression models as the other k-fold techniques. Once
again, the processes for both models was identical apart from the number
of predictors included in the models.
