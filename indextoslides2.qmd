...
3. Use the model on the validation set to create predicted outcome values
4. Compare the predicted values to the actual outcome values to determine model performance

- Can cause unreliable models due to differently distributed training and validation data [@santos2023statistical].
...
- The validation set is comprised of a single observation, so k is equal to the sample size [@shao2016efficient].
- This technique can be ideal with smaller sample sizes, as the lack of randomization when dividing the dataset creates constant accuracy estimates  [@wong2015performance]
- It can be computationally expensive due to the high number of folds for large datasets.
...
Figure 4 demonstrates how AUC uses the positive and negative outcomes to
...
## Conclusion
  ...
  - The full models (which includes all predictor variables within
...
ability compared to the reduced model. 
  - However, the difference in MSEs between the two
models is marginal, but the full model by technicality does have
...
- The rest of the models had only marginal differences in
...
- Ultimately, most of these techniques seem
