...
Leave-one-out cross-validation (LOOCV) is a variation of k-fold cross-validation in that the dataset gets divided into multiple folds to be used for training and validation sets, then the model gets fitted and evaluated until each fold has been used as the validation set. However, for LOOCV, the validation set is comprised of a single observation, so k is equal to the sample size [@shao2016efficient]. This method can be preferable to other methods when the sample size is small because the lack of randomness when dividing the dataset causes the accuracy estimate of the models to be constant [@wong2015performance].
...
k10_m2_auc <- cv.glm(data, m2, K=10, cost=auc)$delta
k10_m2_auc
```
...
#assess_m1_mccv <- collect_predictions(m1_MCCV)
#assess_m1_mccv
#assess_m1_mccv %>% 
  #ggplot(aes(x = DEATH_EVENT, y = .pre)) + 
  #geom_point(alpha = .15) +
