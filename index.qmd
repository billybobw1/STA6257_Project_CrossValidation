...
cv.glm(data, tenfold_m1, K=10)$delta
...
cv.glm(data, tenfold_m2, K=10)$delta
```
...
set.seed(143269)
fivefold_m1 <- glm(DEATH_EVENT ~ age + anaemia + creatinine_phosphokinase + diabetes + ejection_fraction + high_blood_pressure + platelets + serum_creatinine + serum_sodium + sex + smoking + time,
         data = data, 
         family = "binomial"(link = "logit"))

cv.glm(data, fivefold_m1, K=5)$delta


set.seed(185329)
fivefold_m2 <- glm(DEATH_EVENT ~ ejection_fraction + serum_creatinine,
         data = data, 
         family = "binomial"(link = "logit"))

cv.glm(data, fivefold_m2, K=5)$delta
```
