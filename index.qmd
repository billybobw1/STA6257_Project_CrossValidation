...
6.  Calculate the overall test predictors by averaging all k test
    ...
    k=5](images/K-fold-cross-validation-1.png){width="413"}
...
performance is the area under the ROC curve (AUC). AUC conveys how well
a model can differentiate between different classes, and it is
preferable to other metrics, such as accuracy, because it accounts for
both true and false positives [@cantor2000determining], which is demonstrated in Figure 2. 
...
AUC scores
can range from 0 to 1, with 0 indicating the model has no ability to
discern between classes, and 1 indicating the model perfectly predicts
classfication. Figure 6 demonstrates how AUC uses the positive and
negative outcomes to calculate a value that indicates the probability of
a positive result ranking higher than negative results, where $S_p$ is
the total number of positive outcomes ranked, and $n_p$ and $n_n$ are
the number of positive and negative outcomes, respectively
[@hossin2015review].
...
of the heart [@chicco2020machine]. Both creatinine features indicate some right-tailed skewness. These all look fairly normally distributed, but there is some potential skewing for the platelet and serum sodium features.
```{r}
par(mfrow=c(2,4))
...
before the end of the follow-up period. All variables except sex had more negative occurences than positive. The dataset included more males than females. 
```{r}
par(mfrow=c(2,3))
...
will be used with each of our techniques to fit our models. Two
different models will be fit for each technique: one with every
predictor variable included (Model 1), and the other using only serum
creatinine and ejection fraction (Model 2), as was advised by
[@chicco2020machine]. The goal is to compare how the difference in
performance between the two models changes when various cross-validation
techniques are applied. The performance of the models will be determined
using their MSE and AUC values.
...
cross validation analysis to be most effective in most cases. The `cv.glm()` function from the `boot` package will be utilized to perform our cross-validation, where we will specify our dataset, the model, the number of k-folds, and an optional cost function to calculate the performance metric. Our models were created using the `glm()` function, changing the `family=` argument to perform logistic regression. In model 1, all predictor variables are included. In model 2, only ejection
...
predicted using those 2 variables alone. Our performance metrics are calculated by the `$delta` of the `cv.glm()` function. The default metric for `cv.glm()` is MSE, so the cost argument isn't necessary to find our MSE. An AUC cost function was created to add to another `cv.glm()` in order to get this metric as well. This cost function will be used for both k-folds
and LOOCV. 
```{r}
library(boot)
...
10-fold analysis above. The process for creating the models is exactly the same as in the 10-folds techniques, using `glm()` to create logistic regression models with differing numbers of variables. Again, in model 1, all predictor variables are
included. In model 2, only ejection fraction and serum creatinine are
included. The `cv.glm()` function is also being used the same as in the 10-fold techniques, with the only difference being the argument specifying the number of k-folds being changed to five. The performance metrics are also using the delta from `cv.glm()`, with the cost function determining which metric is calculated.
```{r}
library(boot)
...
manually. Here, we are using a 50-50 split, then the `training()` and `testing()` functions created the respective subsets. Our functions for creating
each model were identical to our previous techniques, using `glm()` to
...
After conducting the analysis, it can be seen that model 2 provides
...
observations. Once again, the `$delta` of the `cv.glm()` function is used to create our performance metric, using a cost function to find our AUC
...
For the Monte Carlo technique, logistic regression will be used to
create the models. Unlike previous techniques, a resampling function was
created from the `mc_cv()` function to repeat the splitting of the data many times. Our proportion
of training to validation sets is the standard 70/30 split, and the data
will be resampled 100 times. Workflows were used to specify the logistic regression and the variables being used in each of our models. Once again, the first model contains all twelve predictor variables, and the second model contains only the serum creatinine and ejection fraction variables.
...
earlier to our models to perform our cross-validation. For the MSE of the models, we'll be using
the Brier score as the `fit_resamples()` function generates the Brier
score automatically, and it performs similarly to MSE in classification
problems [@ferro2012bias]. The function also calculates the AUC automatically. The `collect_metrics()` function was used to gather these metrics. 
```{r}
...
techniques, our full model had the lower MSE/ Brier score and higher
AUC. The smaller model's AUC of 0.763 is still acceptable as far as the
metric's ideal scores go, but the increase of about 0.1 indicates there
might be another predictor or two that could contribute significantly to
the model.
...
ejection fraction as predictor variables. The logistic regression
analysis we conducted using various cross-validation techniques has
shown that the full model (which includes all predictor variables within
the dataset) tends to have lower mean squared error (MSE) and higher
Area under the Curve (AUC) values after testing. This essentially means
the full model is more robust with predictive ability compared to the
reduced model. That being said, the difference in MSEs between the two
models is marginal, however, the full model by technicality does have
slightly more predictive ability. It also has a better ability to
distinguish between classes, whether or not the patient died within
...
typically preferred in statistical analysis. Since the AUC values did
show differences between models, the model could perhaps benefit from
some additional predictors. However, the reduced models still
demonstrated acceptable class discrimination.

Single hold-out cross-validation stood out the most as having the worst
performance, with MSE values that were significantly higher than that of
the rest of the techniques. This result indicates that this technique
will not always be the best route to take when trying to validate a
model. The rest of the models had only marginal differences in
performance. However, when considering the process of administering the
techniques, the k-folds, including leave-one-out, provided the most
straight-forward approach. Having a function that performed the
cross-validation from the logistic regression model removed some of the
opportunity for human error. Ultimately, most of these techniques seem
sufficient to help validate a model based on this example.
